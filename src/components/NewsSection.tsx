  ...
  return (
    <section className="py-16 bg-secondary/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-3xl font-bold text-foreground dark:text-white mb-8">Actualités</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {news.map((item, index) => (
            <div key={index} className="bg-accent/50 rounded-lg p-6 hover:bg-accent/70 transition-all duration-300">
              ...
              <h3 className="text-xl font-bold text-foreground dark:text-white mt-2">{item.title}</h3>
              <p className="text-muted-foreground dark:text-gray-200 mt-2">{item.description}</p>
            </div>
          ))}
        </div>
      </div>
